...
class Clcurl
{
	...
	public static function pdf2txt2($filename) {
		...
		foreach ($a_obj as $obj) {
			...
			if (is_array($a_filter)) {
				...
				$a_data = self::getDataArray($obj, "stream\r\n", "endstream");
				if (is_array($a_data)) {
	...
	public static function ps2txt($ps_data) {
		$result = "";
		$a_data = self::getDataArray($ps_data, "[", "]");
		if (is_array($a_data)) {
			foreach ($a_data as $ps_text) {
				$a_text = self::getDataArray($ps_text, "(", ")");
				if (is_array($a_text)) {
			...
			$a_text = self::getDataArray($ps_data, "(", ")");
			if (is_array($a_text)) {
				foreach ($a_text as $text) {
					$result.= substr($text, 1, strlen($text) - 2);
				}
	...
	// Author : Jonathan Beckett, 2005-05-02
	public static function getDataArray($data, $start_word, $end_word) {
