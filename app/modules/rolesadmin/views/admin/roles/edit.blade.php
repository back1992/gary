...
<!-- 				<div class="form-group">
				...
				<div class="form-group">
					@foreach ($permissionsChunk as $chunk)
					<div class="row">
						...
						<div class="col-xs-8 col-sm-4">
							<label>
								...
								<input type="checkbox" id="permissions[{{{ $permission['id'] }}}]" name="permissions[{{{ $permission['id'] }}}]" value="1"{{{ (isset($permission['checked']) && $permission['checked'] == true ? ' checked="checked"' : '')}}} />
								<!-- {{{ $permission['display_name'] }}} -->
								<!-- 				<button type="button" class="btn btn-default">{{{ $permission['display_name'] }}}</button>	 -->					
								<button type="button" {{{ (isset($permission['checked']) && $permission['checked'] == true ? ' class="btn btn-primary"' : ' class="btn btn-default"')}}}>{{{ $permission['display_name'] }}}</button>
							</label>
						</div>
						@endforeach
					</div>
